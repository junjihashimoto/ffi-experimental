...
spec :: Spec
spec = describe "BasicSpec" $ forM_ [kFloat,kDouble] $ \dtype -> do
...
-- }
  ...
  it "TestReisze" $ do
    a <- join $ empty_lo <$> intArray [1,1] <*> options kCPU dtype
